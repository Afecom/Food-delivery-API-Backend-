...
app.patch('/orders/:id/status', admin_auth, updateOrderStatus(models))

describe("signup, login, create order and update order status", () => {
    beforeAll(async () => {
    ...
    it("Should signup, login and create an order and update order status", async () => {
        ...
        const mock_admin_user_signup_data = {
            first_name: "Abebe",
            last_name: "Kebede",
            email: "abekebe@example.com",
            password: "abekebe@123",
            role: "Admin"
        }
        const mock_admin_user_login_data = {
            email: "abekebe@example.com",
            password: "abekebe@123"
        }
        const admin_signup_res = await request(app)
                                .post('/signup')
                                .send(mock_admin_user_signup_data)
                                .expect(201)
        
        const admin_login_res = await request(app)
                                    .post('/login')
                                    .send(mock_admin_user_login_data)
                                    .expect(200)
        const admin_access_token = admin_login_res.body.access_token

        const update_order_status_res = await request(app)
                                                .patch('/orders/1/status')
                                                .set("Authorization", `Bearer ${admin_access_token}`)
                                                .send({status: "preparing"})
                                                .expect(200)
        const update_order_status_res_message = update_order_status_res.body.message
        const update_order_status_res_order = update_order_status_res.body.order
        expect(update_order_status_res_message).toBe('Order status updated successfully')
        expect(update_order_status_res_order).toHaveProperty('id')
        expect(update_order_status_res_order.user_id).toBe(mock_order_data.user_id)
        expect(update_order_status_res_order.restaurant_id).toBe(mock_order_data.restaurant_id)
        expect(update_order_status_res_order.status).toBe('preparing')
    })
})
